...
# gen %%tag contents
$META get.tag:bookmarks{$databox} > %%www/tmp/${session}/tags
...
%%www/bin/bookmarks_page_links.sh $page $pages "$table_command" $num_of_line_per_page > %%www/tmp/${session}/page_link &
wait
