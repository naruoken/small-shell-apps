...
# gen %%tag contents
$META get.tag:inquiries{$databox} > %%www/tmp/${session}/tags
...
%%www/bin/inquiries_page_links.sh $page $pages "$table_command" $num_of_line_per_page > %%www/tmp/${session}/page_link &
wait
...
if [ "$err_chk" ];then
  ...
  echo "<h2>Oops something must be wrong, please check inquiries_table.sh</h2>"
  if [ "$session" ];then
