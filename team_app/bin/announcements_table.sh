...
# gen %%tag contents
$META get.tag:announcements{$databox} > %%www/tmp/$session/tags
...
if [ ! "$user_name" = "guest" ];then
  permission=`$META get.attr:announcements/$user_name{permission}`
else
  permission="ro"
fi

# gen %%page_link contents
%%www/bin/announcements_page_links.sh $page $pages "$table_command" $num_of_line_per_page > %%www/tmp/$session/page_link &
wait
...
if [ "$err_chk" ];then
  ...
  echo "<h2>Oops something must be wrong, please check announcements_table.sh</h2>"
  if [ "$session" ];then
...
if [ "$line_num" = 0 ];then
  if [ "$err_chk" = "" -a "$filter_table" = "-" -a ! "$sort_col" ];then
    ...
    view=announcements_table.html.def
...
| $SED "s/%%params/subapp=announcements\&session=$session\&pin=$pin\&databox=$databox/g"
