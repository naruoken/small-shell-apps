...
if [ "$error_chk" ];then
  cat %%www/descriptor/announcements_del_err.html.def | $SED -r "s/^( *)</</1" \
  ...
  echo "<meta http-equiv=\"refresh\" content=\"0; url=./team?subapp=announcements&session=$session&pin=$pin&req=table&update=yes\">"
fi
