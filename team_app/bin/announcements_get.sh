...
if [ $id = "new" ];then
  ...
  # gen read only datas
  #---------------------------
  #$DATA_SHELL databox:announcements action:get id:$id keys:%%keys format:none > %%www/tmp/$session/dataset.0.1
...
if [ "$master" -a "$permission" = "rw" ];then
  if [ "$redirect" = "no" ];then
    if [ "$id" = "new" ];then
      view="announcements_get_new_master_failed.html.def"
    else
      view="announcements_get_rw_master_failed.html.def"
    fi
  fi
fi
