...
if [ $id = "new" ];then
  ...
  # gen read only datas
  #---------------------------
  #$DATA_SHELL databox:inquiries action:get id:$id keys:%%keys format:none > ../tmp/$session/dataset.0.1
...
# set view
if [ "$error_chk" ];then
  echo "<h2>Oops please something must be wrong, please check  inquiries_get.sh</h2>"

elif [ "$form_chk" = "urlenc" ];then
  if [ "$id" = "new" ];then
    view="inquiries_new.html.def"
  else
    view="inquiries_get.html.def"
  fi
elif [ "$form_chk" = "multipart" ];then
  if [ "$id" = "new" ];then
    view="inquiries_new_incf.html.def"
  else
    view="inquiries_get_incf.html.def"
  fi
fi
