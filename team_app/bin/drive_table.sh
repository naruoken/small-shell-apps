...
# gen %%tag contents
$META get.tag:drive{$databox} > ../tmp/$session/tags
...
../bin/drive_page_links.sh $page $pages $table_command > ../tmp/$session/page_link &
wait
...
if [ "$err_chk" ];then
  ...
  echo "<h2>Oops please define Dawtabox and keys in drive_table.sh for generating table</h2>"
  if [ "$session" ];then
    rm -rf ../tmp/$session
  fi
  exit 1
