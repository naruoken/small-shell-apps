...
if [ "$master" -a "$permission" = "rw" ];then
  if [ "$redirect" = "no" ];then
    if [ "$id" = "new" ];then
      view="bookmarks_get_new_master_failed.html.def"
    else
      view="bookmarks_get_rw_master_failed.html.def"
    fi
  fi
fi
